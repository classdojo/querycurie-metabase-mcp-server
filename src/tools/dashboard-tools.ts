...
export function addDashboardTools(server: any, metabaseClient: MetabaseClient) {
  ...
  server.addTool({
    ...
    execute: async (args: { dashboard_id: number; include_screenshot?: boolean }) => {
      try {
  ...
  server.addTool({
    ...
    execute: async (args: { dashboard_id: number; include_screenshot?: boolean; [key: string]: any }) => {
      try {
  ...
  server.addTool({
    ...
    execute: async (args: { dashboard_id: number; card_ids: number[]; template: string; row?: number; height?: number }) => {
      try {
        const botUserId = parseInt(process.env.METABASE_BOT_USER_ID || '', 10);
        if (!botUserId) {
          throw new Error('METABASE_BOT_USER_ID not configured - cannot verify ownership');
        ...
        const dashboard = await metabaseClient.getDashboard(args.dashboard_id);
        if ((dashboard as any).creator_id !== botUserId) {
          throw new Error(`Cannot modify dashboard ${args.dashboard_id}: owned by user ${(dashboard as any).creator_id}, not bot (${botUserId}). Use copy_dashboard first.`);
        }
        ...
        const templates: Record<string, { size_x: number; col: number }[]> = {
          '2-wide': [
            { size_x: 12, col: 0 },
            { size_x: 12, col: 12 },
          ],
        ...
        let targetRow: number = args.row ?? 0;
        ...
        const foundIds = dashcards.filter((dc: any) => args.card_ids.includes(dc.card_id)).map((dc: any) => dc.card_id);
        const missingIds = args.card_ids.filter((id: number) => !foundIds.includes(id));
        if (missingIds.length > 0) {
          const available = dashcards.map((dc: any) => dc.card_id).join(', ');
        ...
        const cleanedDashcards = updatedDashcards.map((dc: any) => {
          const { card, ...rest } = dc;
          return rest;
        });
        ...
        return JSON.stringify({
          ...
          row: targetRow,
          cards: args.card_ids.map((id: number, i: number) => ({
            card_id: id,
        ...
        throw new Error(
          `Failed to arrange cards on dashboard ${args.dashboard_id}: ${
            error instanceof Error ? error.message : "Unknown error"
          }`
        );
      }
