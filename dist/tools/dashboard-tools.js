...
export function addDashboardTools(server, metabaseClient) {
    ...
    server.addTool({
        ...
        description: "Add a card to a dashboard. The dashboard grid is 24 columns wide. Cards default to full width (size_x=24). For side-by-side layout, use size_x=12 with col=0 and col=12.",
        metadata: { isWrite: true },
        parameters: z.object({
            dashboard_id: z.number().describe("The ID of the dashboard"),
            cardId: z
    ...
    server.addTool({
        ...
        description: "Update a dashboard YOU created. You can only modify dashboards you own. To modify someone else's dashboard, use copy_dashboard to make your own copy first.",
        metadata: { isWrite: true },
        parameters: z.object({
    ...
    server.addTool({
        ...
        description: "Update card layout on a dashboard YOU created. You can only modify dashboards you own. To modify someone else's dashboard, use copy_dashboard first. ⚠️ DANGER: REPLACES ALL dashboard cards - any cards not in the array will be DELETED.",
        metadata: { isWrite: true },
        parameters: z.object({
            dashboard_id: z.number().describe("The ID of the dashboard"),
            cards: z
        ...
        execute: async (args) => {
            try {
                ...
                const existingDashboard = await metabaseClient.getDashboard(args.dashboard_id);
                if (existingDashboard.creator_id !== botUserId) {
                    throw new Error(`Cannot modify dashboard ${args.dashboard_id}: owned by user ${existingDashboard.creator_id}, not bot (${botUserId}). Use copy_dashboard first.`);
                }
    ...
    server.addTool({
        ...
        execute: async (args) => {
            try {
                ...
                throw new Error(`Failed to resize card ${args.card_id} on dashboard ${args.dashboard_id}: ${error instanceof Error ? error.message : "Unknown error"}`);
            }
        },
    });
    /**
